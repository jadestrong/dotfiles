...
(defun my-overlay-fn (path leaf)
  ...
  (let* ((path (mapcar #'avy--key-to-char path))
         ...
         (xpath (cdr leaf)))
    (message "str: %s" str)
    (message "xpath: %s" xpath)
    (xwwp-follw-link-highlight-candidates (xwidget-webkit-current-session) str xpath))
  )
(xwwp-js-def follw-link highlight-candidates (str xpath)
...
var rect = link.getBoundingClientRect();
var item = createLinkItem(link, rect, 'aa');
document.body.appendChild(item);
")

(defun jd-test-callback (candidates)
  "Jd-test callback."
  (message "------callback %s %s" (listp candidates) candidates)
  (unwind-protect
      (avy-read (avy-tree (append candidates nil) avy-keys) #'my-overlay-fn #'avy--remove-leading-chars)))
