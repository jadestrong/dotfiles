...
;;   ;; (message "comment-context %s" (jtsx-comment-context-type beg end))
;;   (let* ((comment-context (jtsx-comment-context-type beg end)))
;;     (pcase comment-context
;;       ('jsx-attribute (jtsx-with-jsx-attribute-comment-style
;;                         (comment-region-default beg end arg)))
;;       ('js-nested-in-jsx (jtsx-with-js-nested-in-jsx-comment-style
;;                            (comment-region-default beg end arg)))
;;       ('jsx (jtsx-with-jsx-comment-style
;;               (comment-region-default beg end arg)))
;;       (_ (comment-region-default beg end arg))))
;;   )
...
(with-eval-after-load 'evil-nerd-commenter
  (defun my/comment-or-uncomment-region (beg end)
    (if (derived-mode-p 'typescript-ts-base-mode)
        (let* ((comment-context (jtsx-comment-context-type beg end)))
          (message "comment-context %s %s %s" comment-context beg end)
          (pcase comment-context
            ('jsx-attribute (progn
                              (message "here? %s" comment-start)
            ...
            ('jsx (jtsx-with-jsx-comment-style
                    (evilnc-comment-or-uncomment-region-internal beg end)))
            (_ (evilnc-comment-or-uncomment-region-internal beg end))))
      (evilnc-comment-or-uncomment-region-internal beg end)))
